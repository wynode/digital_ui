...
import { getForeground, postFile } from "@/apis/main.ts"
...
const Images = () => {
  ...
  const [frontBackground, setFrontBackground] = React.useState<any[]>([])
      ...
      <Scrollable>
        ...
        <Block padding="0 1.5rem">
          <div style={{ display: "grid", gap: "8px", gridTemplateColumns: "1fr 1fr" }}>
            {frontBackground.map((image, index) => {
              return (
                <ImageItem
                  key={index}
                  onClick={() => addObject(`${BACK_API_ASSETS}${image.path}`)}
                  preview={`${BACK_API_ASSETS}${image.path}/?quality=10`}
                />
              )
            })}
          </div>
