  ...
  const getListData = async () => {
    try {
      const res = await getLiveConfig({
        ...
        "filter[config_type][_eq]": "atmosphere",
      })
      const { data } = res
  ...
  const handleEditOk = async (formData: TableData) => {
    try {
      ...
      const fetchPromise = initialData
        ? patchLiveConfig(initialData.id, { ...formData, config_type: "atmosphere" })
        : postLiveConfig({ ...formData, brodcast_assis: params.id, config_type: "atmosphere" })
      await fetchPromise
      handleEditCancel()
      message.success(`${initialData ? "修改" : "添加"}氛围活跃成功`)
      getListData()
    } catch {
      setEditConfirmLoading(false)
    }
