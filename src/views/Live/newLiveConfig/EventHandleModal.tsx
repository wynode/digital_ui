...
import { TableData, replyTypeMap, secondTypeMap, eventMap } from "./liveConfig"
...
const ConfigModal: React.FC<ConfigModalProps> = ({ open, onCancel, onOk, modalLoading, initialData }) => {
  ...
  return (
    <Modal
      open={open}
      destroyOnClose
      title={initialData ? "修改氛围活跃" : "添加氛围活跃"}
    ...
    >
      <Form
        ...
        wrapperCol={{ span: 16 }}
      >
        <Form.Item name="second_type" label="氛围方向" rules={[{ required: true }]}>
          <Select options={Array.from(eventMap).map(([value, label]) => ({ value, label }))} />
        </Form.Item>
            ...
            getFieldValue("reply_type") === "text" || getFieldValue("reply_type") === "text_gpt" ? (
              <Form.Item label="直播话术" name={"reply"} rules={[{ required: true }]}>
                <Input.TextArea
                  ...
                  placeholder="欢迎新访客：当有新人进入直播间，主播与客户交流的话术 主动引导客户：引导的方向一般有如下一些方向，分别是引导关注、引导点赞、引导转发、引导下单，AI会在适时的时间进行引导"
                />
              </Form.Item>
            ) : null
          }
